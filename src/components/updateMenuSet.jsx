...
export function DialogMenuSet({ id }) {
  ...
  useEffect(() => {
    ...
    const newOptions = [{value:null, label:'ประเภทเมนู'},...options]
  ...
  return (
    <Dialog className="border">
      ...
      <DialogContent className="sm:max-w-md  max-h-[40rem] transition-height duration-300 ease-in-out overflow-auto">
        <DialogHeader className="mb-1">
          <DialogTitle>ฟอร์มอัพเดตข้อมูลเมนูเซ็ต</DialogTitle>
          <hr />
        </DialogHeader>
        <div className="grid grid-cols-6 gap-2">
          ...
          <div className="col-span-2">
            <Label htmlFor="quan">จำนวน</Label>
          ...
          <div className="col-span-6 border bg-slate-100 rounded-sm py-2 pl-4">
            <table className="w-full text-start table-auto ">
          ...
          <div className="col-span-3">
            <Label htmlFor="addMenu">เพิ่มเมนู</Label>
            <Select
              styles={{height: 2}}
              options={listMenu.filter(
                (item) => item.typeName === selectedType
              )}
        ...
        <DialogFooter className="sm:justify-center mb-5">
          <DialogClose asChild>
